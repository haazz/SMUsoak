...
public class UserControllerTest extends AbstractRestDocsTests {
    ...
    void GetUserImgTest() throws Exception {
        ...
        for(String mail : userImgRequest.getMailList()) {
            userImageResponses.add(ImgDto.UserImageResponse.builder()
                    .mail(mail)
                    .url("URL")
                    .type("imge/jpeg")
                    .build());
        }
        given(userService.getUserImg(any(ImgDto.UserImgRequest.class)))
                .willReturn(userImageResponses);
        ...
        mockMvc.perform(post("/api/v1/user/imgs")
                ...
                .andDo(
                        restDocs.document(
                                requestFields(
                                        fieldWithPath("mail").type(JsonFieldType.STRING).description("@sangmyung.kr로 끝나는 메일"),
                                        fieldWithPath("mailList").type(JsonFieldType.ARRAY).description("가져오고 싶은 이미지의 mail list")
                                ),
                                responseFields(
                                        fieldWithPath("success").type(JsonFieldType.BOOLEAN).description("성공 여부")
                                ).andWithPrefix("data.[].",
                                        fieldWithPath("mail").type(JsonFieldType.STRING).description("이미지 메일"),
                                        fieldWithPath("url").type(JsonFieldType.STRING).description("이미지 다운로드 링크"),
                                        fieldWithPath("type").type(JsonFieldType.STRING).description("이미지 타입")
                                )
                        )
                );
    }
